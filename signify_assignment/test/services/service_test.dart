  ...
  group("ApiService", () {
    test("getQuote", () async {
      ...
      when(service.getQuotes()).thenAnswer((realInvocation) async {
        ...
        var data = Data(id: "1", quoteText: "It is not in the stars to hold our destiny but in ourselves.", quoteAuthor: "author", quoteGenre: "age", v: 20);
        return QuoteResponse(statusCode: 200, message: "Success", pagination: pagination, totalQuotes: 1, data: [data]);
      });
      final response = await service.getQuotes();
    ...
    test("getNoQuote", () async {
      ...
      when(service.getQuotes()).thenAnswer((realInvocation) async {
        var pagination = Pagination(currentPage: 1, nextPage: 2, totalPages: 2);
        return QuoteResponse(statusCode: 200, message: "Success", pagination: pagination, totalQuotes: 1, data: []);
      });
      final response = await service.getQuotes();
